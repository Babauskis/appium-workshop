class FilterPage < BasePage
  def initialize
    @title = Element.new(:xpath, '//android.widget.TextView[contains(@text, "Audi A1")]')
    @button_apply = Element.new(:id, 'save_filter')
    @button_delete_filter = Element.new(:id, 'delete_filter')
  ...
  def save_filter_click
    @button_apply.click
  end

  def delete_filter_click
    @button_delete_filter.click
  end
end